...
Le sketch est `VETRONIC_RS485/VETRONIC_RS485.ino`. Il conserve AutoConnect comme secours de connexion, Jeedom, la console série et les réponses XML sur le port 9200. Les pages **Pilotage**, **Paramètres**, **Jeedom** et **Réseau & mise à jour** utilisent toutes la même interface VE‑Tronic.
...
5. Régler le courant maximal conformément à l'installation. La consigne manuelle et solaire est plafonnée à 32 A, limite confirmée de l'installation. Les valeurs `max_current…` lues sur la WB-01 restent affichées au diagnostic, car certains firmwares annoncent une contrainte secondaire qui ne représente pas la limite d'installation.
6. Comparer les puissances affichées avec le Deye. Le bloc **Registres et coefficients Deye avancés** permet de régler les sept adresses et leurs coefficients, avec les valeurs historiques comme valeurs initiales. Tous les registres actifs doivent rester dans une plage Modbus FC03 de 125 registres. Activer le troisième MPPT seulement si son registre correspond bien à une entrée PV de votre modèle.
...
- Lecture du bloc Deye qui couvre les registres configurés toutes les 2,5 secondes. Par défaut, le bloc 169–190 contient PV1 186, PV2 187, PV3 188, consommation 178, réseau 169, batterie 190 et SOC 184. Les puissances réseau/batterie sont signées ; positif = import/décharge.
...
- Démarrage si surplus ≥ **1 800 W**. Une réserve de 200 W est retirée avant le calcul de consigne : `max(surplus − 200 W, 0) / tension de calcul`, arrondi vers le bas, entre 6 A et 32 A.
- En cours de charge, un surplus inférieur à `6 × tension` ou une décharge batterie supérieure à 300 W ouvre une fenêtre de **300 secondes**. La décharge est considérée résolue sous 150 W afin d'éviter les bascules dues au bruit. Sous le minimum solaire, la consigne descend à 6 A. À expiration, consigne 0 A, avec attente d'un surplus ≥ 1 800 W et de la fin de la décharge batterie avant redémarrage.
...
- Une réponse WB-01 invalide ou, avec des mesures Deye fraîches, une consommation maison calculée négative provoque toujours une demande d’arrêt au prochain cycle. Une tension WB-01 hors 180–260 V utilise une référence de calcul de 230 V. Le bouton Arrêter reste immédiat, sans délai Deye. Les réponses série sont bornées à deux secondes ; un défaut ou une requête web peut donc décaler un cycle. Il ne s’agit pas d’une boucle temps réel stricte.
- La protection SOC est optionnelle et ne concerne que **Solaire dynamique**. Elle arrête la consigne quand le SOC Deye est inférieur ou égal au seuil d’arrêt, puis ne permet la reprise qu’au seuil de reprise. L’interface impose au moins **5 %** d’hystérésis. Une mesure Deye absente conserve le verrou s’il était déjà actif.
- Un état Wi-Fi transitoire ne provoque plus de redémarrage automatique de l’ESP32 : AutoConnect gère la reconnexion et la temporisation peut continuer. Après un véritable redémarrage ou le retour de la WB-01, le solaire dynamique est préparé avec une consigne initiale de 0 A et attend des mesures fraîches avant toute charge.
...
La configuration Deye est conservée dans un espace Preferences séparé de Jeedom/Wi-Fi. Après redémarrage d'un ESP32 qui pilotait la charge, le solaire dynamique est sélectionné, mais la borne reçoit d'abord 0 A. Le firmware vérifie la configuration Deye, les mesures et la préparation de la WB-01 avant de réguler. Une première installation demande donc encore de configurer Deye et confirmer les mesures.

La section **LED RGB intégrée LILYGO** de `/parametres` permet d’activer ou désactiver la LED, de choisir le clignotement en défaut et de définir les quatre couleurs. Les couleurs d’origine sont bleu (démarrage/attente), vert (communications prêtes), violet (charge) et rouge (défaut).
...
La production Deye seule ne valide pas les mesures WB-01. « Courant VE — » indique que la lecture `evse_state` / `$GG*B2` n'est pas valide ou a expiré. Le régulateur vérifie les courants (0–63000 mA) et l'état (0–2). Une tension hors 180–260 V est signalée et calculée sur la référence de 230 V.

...
<details><summary>Registres et coefficients Deye avancés</summary><p class="muted">Changer ces valeurs uniquement après vérification avec la documentation ou les trames de votre onduleur. Tous les registres actifs doivent tenir dans un même bloc de 125 registres.</p>
<label>Réseau : registre <input name="regGrid" type="number" min="0" max="65535" required> · coefficient <input name="gridScale" type="number" min="0.001" max="100" step="0.001" required></label>
<label>Consommation : registre <input name="regLoad" type="number" min="0" max="65535" required> · coefficient <input name="loadScale" type="number" min="0.001" max="100" step="0.001" required></label>
<label>Batterie : registre <input name="regBat" type="number" min="0" max="65535" required> · coefficient <input name="batScale" type="number" min="0.001" max="100" step="0.001" required></label>
<label>SOC : registre <input name="regSoc" type="number" min="0" max="65535" required> · coefficient <input name="socScale" type="number" min="0.001" max="100" step="0.001" required></label>
<label>PV 1 : registre <input name="regPv1" type="number" min="0" max="65535" required> · coefficient <input name="pv1Scale" type="number" min="0.001" max="100" step="0.001" required></label>
<label>PV 2 : registre <input name="regPv2" type="number" min="0" max="65535" required> · coefficient <input name="pv2Scale" type="number" min="0.001" max="100" step="0.001" required></label>
<label>PV 3 : registre <input name="regPv3" type="number" min="0" max="65535" required> · coefficient <input name="pv3Scale" type="number" min="0.001" max="100" step="0.001" required></label></details>
...
<fieldset><legend>Protection de batterie pour le mode Solaire dynamique</legend>
<label><input name="socGuard" type="checkbox"> Bloquer la charge solaire selon le SOC de la batterie Deye</label>
<label>Arrêter à ou sous (%) <input name="socStop" type="number" min="0" max="95" required></label>
<label>Reprendre à ou au-dessus (%) <input name="socResume" type="number" min="5" max="100" required></label>
<small>La reprise doit être au moins 5 % au-dessus du seuil d'arrêt. Ce verrou ne concerne pas les modes Manuel et Borne / Jeedom.</small></fieldset>
...
<section data-settings hidden><h2>LED RGB intégrée LILYGO</h2>
<form id="ledSettings"><label><input name="ledOn" type="checkbox"> Activer la LED d'état</label>
<label><input name="ledBlink" type="checkbox"> Faire clignoter la LED lors d'un défaut de communication</label>
<label>Démarrage / attente <input name="ledStart" type="color"></label>
<label>Communications Deye et WB-01 prêtes <input name="ledReady" type="color"></label>
<label>Véhicule en charge <input name="ledCharge" type="color"></label>
<label>Défaut de communication <input name="ledFault" type="color"></label>
<button>Enregistrer la LED</button></form>
<small>Les couleurs sont appliquées immédiatement. Bleu, vert, violet et rouge restent les réglages d'origine.</small></section>
...
if($('settings'))$('settings').onsubmit=e=>{e.preventDefault();action(async()=>{const data=Object.fromEntries(new FormData(e.target));for(const k of ['pv3','includes','meter','socGuard'])data[k]=e.target.elements[k].checked?'1':'0';message(await api('/api/config',data));await status()})};
if($('ledSettings'))$('ledSettings').onsubmit=e=>{e.preventDefault();action(async()=>{const data=Object.fromEntries(new FormData(e.target));for(const k of ['ledOn','ledBlink'])data[k]=e.target.elements[k].checked?'1':'0';message(await api('/api/led',data));await status()})};

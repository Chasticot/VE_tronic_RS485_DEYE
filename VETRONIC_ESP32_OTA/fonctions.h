...
String Read_Evse(String commande)
{
  ...
  const uint32_t transactionStarted = millis();

  while (step == READ_EVSE)
  {
    if (millis() - transactionStarted >= 2000 || cpt_buffer_com >= BUFFERSIZE - 1) {
      return "Erreur : réponse série incomplète (délai ou taille dépassés)";
    }
    yield();
    if (COM->available())
    { // Test if COM answer ?
      while (COM->available() && cpt_buffer_com < BUFFERSIZE - 1 && millis() - transactionStarted < 2000)
      {
...
String DoRepXml(String req, String erreur, String reponse)
{
  ...
  {                               // teste si la reponse vetronic = OK = 200
    ...
    //*************************************************************************************************
    // Commande $MODE = Mode de pilotage ESP32 (Jeedom)
    //*************************************************************************************************
    else if (req.startsWith("$MODE "))
    {
      if (reponse_temporaire.startsWith("$MODE OK "))
        message = reponse_temporaire;
      else {
        message = reponse_temporaire;
        erreur = String("400");
      }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
// Les lectures et commandes courantes restent publiques. Les commandes de
// configuration arbitraires exigent les identifiants des paramètres.
bool legacyPublicCommand(String command)
{
  command.trim();
  if (command.indexOf('\r') >= 0 || command.indexOf('\n') >= 0 || command.indexOf(';') >= 0) return false;
  if (command == "" || command == "list" || command == "date" || command == "help" ||
      command == "version" || command == "evse_state" || command == "$GG*B2" || command == "info") return true;
  // Les commandes $MODE sont validées et exécutées côté ESP32, pas transmises
  // telles quelles à la WB-01. Elles restent compatibles avec Jeedom/HTTP.
  if (command.startsWith("$MODE ")) return true;
  if (command.startsWith("get ") || command.startsWith("help ") || command.startsWith("info ") || command.startsWith("$FP ")) return true;
  if (command.startsWith("$SC ")) {
    int amps; char extra;
    return sscanf(command.c_str(), "$SC %d%c", &amps, &extra) == 1 && amps >= -1 && amps <= 63;
  }
  return false;
}
...
{
  // Pas de popup sur le port XML. Auth uniquement pour modifier des paramètres.
  String expected = "Basic " + base64::encode(String(WEB_USER) + ":" + WEB_PASSWORD);
  String received;
  for (int start = str.indexOf("\r\n") + 2; start > 1 && start < (int)str.length();) {
    int end = str.indexOf("\r\n", start); if (end < 0 || end == start) break;
    String line = str.substring(start, end); int colon = line.indexOf(':');
    String name = line.substring(0, colon); name.toLowerCase();
    if (name == "authorization") { received = line.substring(colon + 1); received.trim(); }
    start = end + 2;
  }
  str = urldecode(str);
  ...
  if (!legacyPublicCommand(Req) && received != expected) {
    TCPClient.print("HTTP/1.1 403 Forbidden\r\nContent-Length: 0\r\nConnection: close\r\n\r\n");
    TCPClient.stop(); return "";
  }
  return Req;
}
